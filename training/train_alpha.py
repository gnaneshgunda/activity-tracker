"""Train the joint alpha + label predictor.

Usage:
    python train_alpha.py --cap 1500 --epochs 30 --out checkpoints/lstm_alpha.pt
"""
...
def build_split_for_group(group, *, cap: int, seed: int, balance_users: bool) -> ds.WindowSet:
    """Build a subject-wise split window set for one group."""
...
def main() -> None:
    ...
    ap.add_argument("--cap", type=int, default=500, help="minutes per class per split")
    ...
    ap.add_argument("--workers", type=int, default=2,
                    help="parallel data-loading workers (keep low on RAM-constrained machines)")
    ap.add_argument("--resume", default=None, help="checkpoint path to resume/fine-tune from")
    ap.add_argument("--out", default="checkpoints/lstm_alpha.pt")
    ap.add_argument("--report", default="checkpoints/lstm_alpha_report.json")
    args = ap.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(message)s")
    log = logging.getLogger("train_alpha")

    # cap data workers before any forking happens
    import os
    ...
    test_metrics = evaluate(
        model,
        norm.apply(sets["test"].X).astype(np.float32),
        sets["test"].y,
        classes=TARGET_CLASSES,
        absent=result.absent_classes,
    )
